			...
			case ViewAngle2DOT5:
				...
				if (this.keyInputHandler.attack.isPressed()
					...
					case LEFT:
						...
						Vector2 endPointLeft = TileMap.entities.get("playerAttackLeft").position
								.sub(new Vector2(TileMap.entities.get("playerAttackLeft").getWidth()
										* TileMap.entities.get("playerAttackLeft").getScale().x / 2
										- Player.ATTACK_OFFSETX, 0))
								.add(new Vector2(-ATTACK_LERP, ATTACK_LERP * TileMap.modifier));
						if (FloatCompare.isLessOrEqual(CollisionBox.collisionBoxs.get("playerAttackLeft").leftUpPoint.x,
								endPointLeft.x)
								&& FloatCompare.isBiggerOrEqual(
										CollisionBox.collisionBoxs.get("playerAttackLeft").leftUpPoint.y,
										endPointLeft.y)) {
					...
					case RIGHT:
									...
									new CollisionBox(
											playerAttackRightAttackEntity.position.sub(new Vector2(0,
													playerAttackRightAttackEntity.getHeight()
															* playerAttackRightAttackEntity.getScale().y / 2
															- Player.ATTACK_OFFSETY)),
											playerAttackRightAttackEntity.position
													.add(new Vector2(playerAttackRightAttackEntity.getWidth()
															* playerAttackRightAttackEntity.getScale().x / 2
															- Player.ATTACK_OFFSETX, 0))));
						}
						...
						if (FloatCompare.isBiggerOrEqual(
								CollisionBox.collisionBoxs.get("playerAttackRight").leftUpPoint.x, endPointRight.x)
